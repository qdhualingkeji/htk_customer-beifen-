...
public class CollectionListRecommendAdapter extends BaseQuickAdapter<ShopAndRecommendEntity.DataBean.RecommendShopListBean,BaseViewHolder> {

    public CollectionListRecommendAdapter(@LayoutRes int layoutResId, @Nullable List<ShopAndRecommendEntity.DataBean.RecommendShopListBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, ShopAndRecommendEntity.DataBean.RecommendShopListBean item) {
        ...
        if (distance == 0){
            ...
            if (distance >= 1000)
                helper.setText(R.id.tv_distance , String.format(mContext.getString(R.string.join_distance_km) ,
                    distance/1000));
            else helper.setText(R.id.tv_distance , String.format(mContext.getString(R.string.join_distance) ,
                    distance));
        }
