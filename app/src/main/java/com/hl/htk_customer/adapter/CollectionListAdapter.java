...
public class CollectionListAdapter extends BaseQuickAdapter<ShopAndRecommendEntity.DataBean.ShopListBean,BaseViewHolder> {

    public CollectionListAdapter(@LayoutRes int layoutResId, @Nullable List<ShopAndRecommendEntity.DataBean.ShopListBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, ShopAndRecommendEntity.DataBean.ShopListBean item) {
        ...
        if (distance == 0){
            ...
            if (distance >= 1000)
                ...
                helper.setText(R.id.tv_distance , String.format(mContext.getString(R.string.join_distance) ,
                        distance));
        }
